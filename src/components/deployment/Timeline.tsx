...
interface TimelineProps {
  ...
  deploymentId: string;
  onAddComment: (comment: { text: string; importance: ImportanceType }) => Promise<void>;
}
...
const Timeline: React.FC<TimelineProps> = ({ comments, deploymentId, onAddComment }) => {
  ...
  // Helper function to get user display name from either a string or user object
  const getUserDisplayName = (user: any): string => {
    ...
    // If user is an object with name or email properties
  ...
  const PaginationControls = () => (
    totalPages > 1 ? (
      <div className="flex items-center justify-center space-x-2 border-t pt-4 pb-2">
        <button 
          onClick={prevPage} 
          disabled={currentPage === 1}
          className={`flex items-center px-2 py-1 rounded ${currentPage === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:bg-indigo-50'}`}
        >
          <ChevronLeft className="h-4 w-4" />
        </button>
        
        <div className="flex space-x-1">
          {/* Show page numbers with ellipsis if there are many pages */}
            ...
            .map((page, index, array) => {
              ...
              return (
                <React.Fragment key={page}>
                  {showEllipsisBefore && <span className="px-2 py-1 text-gray-500">...</span>}
                  <button
                  ...
                  {showEllipsisAfter && <span className="px-2 py-1 text-gray-500">...</span>}
                </React.Fragment>
              );
            })
          }
        ...
        <button 
          ...
          className={`flex items-center px-2 py-1 rounded ${currentPage === totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:bg-indigo-50'}`}
        >
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between border-b pb-2">
                <span className="text-sm text-gray-500">
                  {t('showing')} {indexOfFirstComment + 1}-{Math.min(indexOfLastComment, sortedComments.length)} {t('of')} {sortedComments.length} {t('comments')}
                </span>
                <PaginationControls />
              </div>
            </div>
